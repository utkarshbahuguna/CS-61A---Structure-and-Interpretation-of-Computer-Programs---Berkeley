...
class ProfessorCard(Card):
    ...
    def effect(self, other_card, player, opponent):
        ...
        opponent_deck_copy = opponent.deck.copy()
        discarded = 0
        for card_index in range(len(opponent.deck.cards)):
            if opponent.deck.cards[card_index].attack == other_card.attack or opponent.deck.cards[card_index].defense == other_card.defense:
                opponent_deck_copy.cards.pop(card_index - discarded)
                discarded += 1
