...
# https://inst.eecs.berkeley.edu/~cs61a/fa19/lab/lab05/

# City 

from math import sqrt
def distance(city1, city2):
    """ Return the Eucledian distance between two ciites.
...
def add_chars(w1, w2):
    ...
    You may assume that w1 is a subsequence of w2. Use recursion.
    The letters in the string you return should be in the order you have to add them from left to right.
    If there are multiple characters in the w2 that could correspond to characters in w1, use the leftmost one.
...
def add_trees(t1, t2):
    """ takes in two trees and returns a new tree where each corresponding node from the first tree is added with the node from the second tree.
    ...
    branches_length_t1, branches_length_t2 = len(branches(t1)), len(branches(t2))
    longer_branch_length = max(branches_length_t1, branches_length_t2)
    
    # Making number of branches at a node equal by adding empty nodes
    t1_branches = branches(t1) + [tree(0)] * (longer_branch_length - branches_length_t1)
    t2_branches = branches(t2) + [tree(0)] * (longer_branch_length - branches_length_t2)
