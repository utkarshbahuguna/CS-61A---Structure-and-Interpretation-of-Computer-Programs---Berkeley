; https://inst.eecs.berkeley.edu/~cs61a/fa19/hw/hw07/
...
(define (longest-increasing-subsequence lst)
    ; Returns the longest subsequence in lst in which all the terms are increasing.
...
(define (make-exp base exponent)
  (cond ((eq? exponent 0) 1)
        ((eq? exponent 1) base)
        ((> exponent 1) (cond ((number? base) (expt base exponent))
                              (else `(^ ,base ,exponent))))
        (else 'Error))
)
